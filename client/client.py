...
async def receive_chunks(reader, dest_path):
    ...
    # Receive and verify the file-level checksum
    file_checksum = (await reader.readexactly(64)).decode()
    calculated_checksum = calculate_file_checksum(dest_path)
    if file_checksum != calculated_checksum:
        raise ValueError(f"File checksum mismatch! Received: {file_checksum}, Calculated: {calculated_checksum}")

    logger.info("Received and verified file from server.")
